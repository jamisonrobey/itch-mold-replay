...
int main(const int argc, char** argv) {
  ...
  cli.add_option("--request-address", request_address, "Request server address")
     ->capture_default_str();

  cli.add_option("--request-port", request_port, "Request server port")
     ->check(CLI::Range(1025, 65535))
     ->capture_default_str();
  ...
  cli.add_option("--start-phase,--phase",
                 start_phase,
  ...
  try {
    const Server server{
        ...
        loopback,
        request_address,
        static_cast<std::uint16_t>(request_port),
        replay_speed,
        start_phase
    };
    server.start();
